...
public class ChatQuickMessageButtons {
    ...
    public ChatQuickMessageButtons(@NotNull Minecraft client, int width, int height) {
        this.client = client;
        this.width = width;
        this.height = height;
    }
    ...
    public ArrayList<TransparentButton> getAllButton(@NotNull ButtonClickListener listener) {
        ...
        if (this.options.chatQuickMessageButton) {
            int chatWidth = this.client.gui.getChat().getWidth();
            int chatHeight = this.client.gui.getChat().getHeight();
    ...
    private void addButton(@NotNull ButtonClickListener listener, int width, int height, ArrayList<TransparentButton> buttons) {
        ...
        this.options.message.forEach(message -> {
            ...
            buttons.add(new TransparentButton.Builder(Component.literal(QuickChatUtils.getAbbreviatedText(message, this.client, buttonWidth - 6)),
